-- Update search_medications to include fuzzy matching for typos
...
AS $$
  ...
  WHERE (
    ...
    -- Fuzzy matching using pg_trgm similarity (threshold 0.3)
    OR similarity(m.generic_name, search_query) > 0.3
    OR similarity(COALESCE(array_to_string(m.brand_names, ' '), ''), search_query) > 0.3
  )
  ORDER BY 
